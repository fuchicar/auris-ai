...
func readAPIKey(t *testing.T) string {
	t.Helper()
	data, err := os.ReadFile("test_data/openai_api_key")
	if err != nil {
		t.Skip("no API key in test_data/openai_api_key; skipping integration test")
	}
	key := strings.TrimSpace(string(data))
	if key == "" {
		t.Skip("empty API key in test_data/openai_api_key; skipping integration test")
	}
	return key
}

// readBaseURL reads an optional base URL override, used to point the
// integration tests at any OpenAI-compatible third party (MiniMax, DeepSeek,
// a self-hosted proxy, ...) instead of the real OpenAI API. Unlike the API
// key, its absence does not skip the test — it just means the driver's own
// default (the real OpenAI endpoint) applies.
func readBaseURL(t *testing.T) string {
	t.Helper()
	data, err := os.ReadFile("test_data/openai_base_url")
	if err != nil {
		return ""
	}
...
func newConnectedDriver(t *testing.T) *openai.Driver {
	t.Helper()
	var opts []openai.Option
	if baseURL := readBaseURL(t); baseURL != "" {
		opts = append(opts, openai.WithBaseURL(baseURL))
	}
	d := openai.New(readAPIKey(t), opts...)
...
// preferredModel returns gpt-4o-mini if available (cheap, real OpenAI), else
// the first model found — which naturally picks up whatever a third-party
// OpenAI-compatible provider (e.g. MiniMax) reports via ListModels when the
// base URL is overridden.
func preferredModel(t *testing.T, d *openai.Driver) llm.Model {
	t.Helper()
	...
	for _, m := range models {
		if strings.EqualFold(m.ID, "gpt-4o-mini") {
			return m
		}
	}
	return models[0]
...
func TestComplete_PreferredModel(t *testing.T) {
	d := newConnectedDriver(t)
...
func TestStream_PreferredModel(t *testing.T) {
	d := newConnectedDriver(t)
	model := preferredModel(t, d)
