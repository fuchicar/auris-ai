...
- `market_chain.go` — `NewMarketChain(providers ...market.ProviderAPI) market.ProviderAPI` wraps an ordered list of providers (`providers[0]` primary) into a single `market.ProviderAPI`. Each call is tried against providers in order; it falls back to the next only on `ErrNotFound`/`ErrNotSupported`/`ErrRateLimit`/`ErrSubscriptionRequired` — other errors (`ErrUnauthorized`, `ErrNotConnected`, ...) surface immediately rather than being masked by a fallback. If none resolve the call, the **primary's** error is returned (never the last-tried provider's). For the list/search-shaped calls (`SearchInstrument`, `ListInstruments`, `GetCandles`, `GetCorporateActions`), a provider's successful-but-empty result also cascades to the next provider — this is what lets EODHD answer BME symbols like `BKT.MC` after FMP's search legitimately returns `200 + []` for them (FEAT-7), while still returning the primary's own outcome if no provider yields data. `pkg/tui/app.go`'s `buildMarketProvider` constructs this chain from every market provider the user configured, in `registry.AllMarket()` order (see FEAT-7 in `docs/task_completed.md`).
...
- `FinancialProfile` is always stored as plaintext JSON (not credentials).
- **Storage encryption (FEAT-16)**: portfolios and chat sessions optionally get the same AES-256-GCM treatment, gated by `AurisConfig.EncryptStorage` — **on by default for new setups** (opt-out, set in `pkg/tui`'s `ScreenPassphrase` transition; toggled from the menu via `ScreenEncryption`/`/encryption`). Unlike the credential-encryption salt above, `AurisConfig.StorageSalt` is generated once and stays stable across saves — regenerating it would silently change the derived key underneath already-encrypted files. The active key is held in package-level state (`config.SetStorageKey`/`StorageKey`, mirroring the `portfoliosDirOverride` test-override convention) rather than threaded through every call site; `nil` means plaintext. `config.MarshalEncryptable`/`UnmarshalEncryptable` wrap/detect an `EncryptedEnvelope` (`{"encrypted":true,"data":"..."}`) so legacy plaintext files, freshly encrypted files, and (mid-toggle) a mix of both all load through the same code path. Toggling on/off, or changing the passphrase while enabled, synchronously re-keys every existing file via `portfolio.ReencryptAllPortfolios`/`config.ReencryptAllSessions` (`AppModel.setStorageEncryption`) — idempotent by design (a file already in the target state is detected and left alone) so a failed batch can be retried.
...
- **58 tools** and the per-category counts (21 calculation, 16 portfolio, 10 market, 6 technical indicators, 3 time, 1 news, 1 currency) → `buildTools()` in `pkg/agent/tools.go` / `TestBuildTools_Count`. If you change the tool count, also rescale the category bar widths (`.toolcat .bar i`, sized relative to the largest category).
- **692 test functions / 171 in pkg/finance** → recount with `grep -rn "func Test" --include="*_test.go" | wc -l`. The calculation-engine tests live in `pkg/finance` since REF-7 (previously counted under `pkg/agent`).
- **5 AI drivers (Anthropic · Gemini · Ollama · MiniMax · OpenAI) and 2 market drivers (FMP · EODHD, cascade with fallback; plus the simulation driver, DD-7)** → `pkg/registry/`. A new driver changes slides 5, 6 and possibly 3/15.
- **~22,800 LOC (non-test) · 20 packages · Go 1.25 · 32 TUI screens · 2 locales · 6 themes** → recount when they drift meaningfully.
